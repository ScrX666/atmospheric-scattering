...
{
	#if UNITY_REVERSED_Z
	real depth = SampleSceneDepth(i_UV);
	#else
	// 调整 z 以匹配 OpenGL 的 NDC
	real depth = lerp(UNITY_NEAR_CLIP_VALUE, 1, SampleSceneDepth(UV));
	#endif
	float3 positionWorldSpace = ComputeWorldSpacePosition(i_UV, depth, UNITY_MATRIX_I_VP);
...
{
	#if UNITY_REVERSED_Z
	real deviceZ = SampleSceneDepth(i.uv);
	#else
	// 调整 z 以匹配 OpenGL 的 NDC
	real deviceZ = lerp(UNITY_NEAR_CLIP_VALUE, 1, SampleSceneDepth(UV));
	#endif
